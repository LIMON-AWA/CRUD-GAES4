/*
 * Click nbfs://nbhost/SystemFileSystem/Templates/Licenses/license-default.txt to change this license
 * Click nbfs://nbhost/SystemFileSystem/Templates/Classes/Class.java to edit this template
 */
package com.gaesprog.MavenProjecto.app.facadeimp;

import com.gaesprog.MavenProjecto.app.model.Factura;
import com.gaesprog.MavenProjecto.app.utilities.JPAUtil;
import java.util.ArrayList;
import java.util.List;
import javax.persistence.EntityManager;
import javax.persistence.Persistence;
import javax.persistence.Query;
import com.gaesprog.MavenProjecto.app.facade.IntFactura;

/**
 *
 * @author Administrator
 */
public class FacturasIMP implements IntFactura {

    EntityManager entity = JPAUtil.getEntityManagerFactory().createEntityManager();

    List<Factura> ListFactura = new ArrayList<Factura>();
    
    @Override
    public List<Factura> findAll() {
        try{
            this.entity.getTransaction().begin();
            Query q = this.entity.createQuery("SELECT F FROM Facturas F");
            this.ListFactura = q.getResultList();
        } catch(Exception e) {
            JPAUtil.shutdown();
        } finally {
            if (this.entity != null){
                //JPAUtil.shutdown();
            }
        }
        return this.ListFactura;
    }  
    
    
    @Override
    public Factura findById(long id) {
        Factura Factura=new Factura();
        try {
            this.entity.getTransaction().begin();
            Factura=this.entity.find(Factura.class, id);
        } catch (Exception e) {
            e.printStackTrace();
        }
        return Factura;
    }
    
    @Override
    public void add(Factura Factura){
        try{
            this.entity.getTransaction().begin();
            this.entity.persist(Factura);
            this.entity.getTransaction().commit();
        } catch (Exception e){
            JPAUtil.shutdown();
        } finally {
            if (this.entity != null){
                //JPAUtil.shutdown();
            }
        }
    }
    
    @Override
    public void update(Factura Factura){
        try{
            this.entity.getTransaction().begin();
            this.entity.persist(Factura);
            this.entity.getTransaction().commit();
        } catch (Exception e){
            JPAUtil.shutdown();
        } finally {
            if (this.entity != null){
                //JPAUtil.shutdown();
            }
        }
    }
    
    @Override
    public void delete(Factura Factura){
        try{
            this.entity.getTransaction().begin();
            this.entity.persist(Factura);
            this.entity.getTransaction().commit();
        } catch (Exception e){
            JPAUtil.shutdown();
        } finally {
            if (this.entity != null){
                //JPAUtil.shutdown();
            }
        }
    }
    }